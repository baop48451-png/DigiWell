...
export const exportWaterDataCSV = (userId: string, history: DailyWaterSummary[]) => {
  const headers = ['Ngay', 'Tong Luong Nuoc (ml)', 'Lan uong', 'Hoan thanh (%)'];
  const rows = history.map(h => [
    h.day,
    h.totalMl.toString(),
    h.entryCount.toString(),
    Math.round(h.percentage).toString() + '%'
  ]);
  ...
  link.setAttribute("download", `digiwell_history_${userId}_${new Date().toISOString().split('T')[0]}.csv`);
