  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-hide">
        {chatMessages.length === 0 && !isChatLoading && (
          <div className="flex flex-col items-center justify-center h-full opacity-50 space-y-4">
            ...
            <p className="text-slate-400 text-sm text-center">Hãy thử: "Ghi nhận giúp tôi 1 ly trà đào 300ml"<br/>hoặc hỏi về chế độ dinh dưỡng.</p>
          </div>
        )}
        ...
        {isChatLoading && (
          <div className="flex justify-start items-end">
            <div className="w-8 h-8 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center mr-2 flex-shrink-0 mb-1"><Bot size={14} className="text-indigo-400" /></div>
            <div className="max-w-[75%] p-4 rounded-2xl bg-slate-800 border border-slate-700 text-slate-400 rounded-bl-sm flex gap-1.5 items-center h-[46px]"><div className="w-2 h-2 rounded-full bg-indigo-400/70 animate-bounce" /><div className="w-2 h-2 rounded-full bg-indigo-400/70 animate-bounce" style={{ animationDelay: '0.15s' }} /><div className="w-2 h-2 rounded-full bg-indigo-400/70 animate-bounce" style={{ animationDelay: '0.3s' }} /></div>
          </div>
        )}
      ...
      <div className="p-4 bg-slate-900/95 border-t border-slate-800 backdrop-blur-xl pb-8">
        <form onSubmit={handleSendChatMessage} className="flex gap-2"><input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Hỏi AI hoặc nhờ thêm nước..." className="flex-1 bg-slate-800/80 border border-slate-700 rounded-full px-5 py-3.5 text-sm text-white outline-none focus:border-cyan-500 shadow-inner" /><button type="submit" disabled={isChatLoading || !chatInput.trim()} className="w-[50px] h-[50px] rounded-full bg-cyan-500 flex items-center justify-center text-slate-900 disabled:opacity-50 active:scale-95 transition-all shadow-[0_0_15px_rgba(6,182,212,0.3)] flex-shrink-0"><Send size={20} className="ml-1" /></button></form>
      </div>
    </div>
  );
};
